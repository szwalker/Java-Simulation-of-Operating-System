...
Description:
    ...
    * This program simulates High Penalty Ratio Next
...
class Job:
    ...
    def status_check(self,time):
        ...
        if self.state == 'unstarted' and time >= self.A:
            ...
            cpu_can_run = True
        try:
            self.penalty_ratio = 1 / ((time - self.first_ready_time) / max(1, self.total_cpu_time))
        except Exception:
            self.penalty_ratio = 1
...
# filter the ready jobs and add to PQ following tie-break rules
def scan_job(PQ,lst,time):
    join_queue = []
    for j in lst:
        j.status_check(time)
        if j.is_ready():
...
if __name__ == "__main__":
    ...
    while job_incomplete > 0:
        ...
        scan_job(PQ,jobs_lst,time)

        if not PQ.is_empty() and cpu_can_run:
            cur_job = PQ.extract_min()
            cur_job.start_run()
