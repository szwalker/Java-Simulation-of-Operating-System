...
Description:
    ...
    * This program simulates Round Robin (with quantum 2).
...
class Job:
    ...
    def status_check(self,time):
        ...
        if self.state == 'unstarted' and time >= self.A:
            ...
            self.state = 'blocked'
            cpu_can_run = True
        elif self.state == 'running' and self.quantum_remaining == 0:
            self.state = 'ready'
            cpu_can_run = True
    ...
    def status_report(self):
        ...
        if self.state == 'running':
            return self.state + ' ' + str(self.quantum_remaining+1)
    ...
    def start_run(self):
        ...
        cpu_can_run = False
        self.state = 'running'
        self.quantum_remaining = 2 # Round Robin with quantum equals to 2
        ...
        # in case if the quantum is greater than the time needed to complete job
        if self.total_cpu_time + self.quantum_remaining > self.C:
            # then switch to the number just necessary to complete job
            self.quantum_remaining = self.C - self.total_cpu_time
        elif self.quantum_remaining > self.cpu_burst_remaining:
            self.quantum_remaining = self.cpu_burst_remaining
